...
const OPENAI_COMPATIBLE_SPEC_OVERRIDES: BuiltinSpecOverride[] = [
	...
	},
	{
		...
		description:
			"Local OpenAI-compatible server routed via FreeLLMAPI. The 'auto' model picks whatever is available; falls back to OpenRouter when the local server is unreachable.",
		family: "openai-compatible",
		popular: 45,
		capabilities: ["reasoning", "prompt-cache", "tools"],
		defaultModelId: FREELLM_DEFAULT_MODEL_ID,
		apiKeyEnv: ["FREELLM_API_KEY"],
		modelsProviderId: "freellmapi",
		defaults: { baseUrl: FREELLM_BASE_URL },
		modelsSourceUrl: `${FREELLM_BASE_URL}/v1/models`,
		configFields: [API_KEY_FIELD, BASE_URL_FIELD],
	},
	{
	...
	},
	{
		...
		popular: 20,
		capabilities: ["reasoning", "prompt-cache"],
		defaultModelId: OPENROUTER_FALLBACK_MODEL_ID,
		apiKeyEnv: [OPENROUTER_API_KEY_ENV],
		modelsProviderId: "openrouter",
		docsUrl: "https://openrouter.ai/models",
		defaults: { baseUrl: OPENROUTER_BASE_URL },
		metadata: {
			...ANTHROPIC_AND_QWEN_CACHE_ROUTING_METADATA,
